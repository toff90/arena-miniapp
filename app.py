...
# NOTE: In-memory dedup doesn't work with multiple gunicorn workers.
# Protection against double-counting is handled client-side:
#   1. No sendBeacon — only localStorage for offline recovery
#   2. Math.max reconciliation — displayed score never goes backwards
#   3. Server uses optimistic locking via last_energy_update timestamp
