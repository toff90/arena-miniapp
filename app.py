"""
Arena MiniApp API v3.1 - Atomic Coin Operations
=================================================
Changes vs v3.0:
  #1 CRITICAL: Atomic SQL operations via Supabase RPC (eliminates race conditions)
  #2 CRITICAL: tap_earn_atomic RPC for /api/tap
  #3 CRITICAL: claim_daily_atomic RPC for /api/daily
  #4 CRITICAL: add_referral_passive RPC for referral income
  #5 MEDIO:    Fallback to old sb_patch method if RPC fails
  #6 MEDIO:    Startup banner to verify deployed version
  All previous v3.0 fixes still included (server-side coin calc, rate limiting, etc.)
"""
...
@app.route("/")
def index():
    return "Arena API v3.1 - Atomic Coin Operations", 200

@app.route("/health")
def health():
    return jsonify({"status": "ok", "version": "3.1", "atomic_rpc": True}), 200
...
def record_taps():
    try:
        ...
        if rpc_result:
            # RPC succeeded - atomic update, no race condition possible
            logger.info(f"[TAP] {wallet[:8]}... +{final_coins} coins, -{actual_taps} energy (RPC atomic)")
            server_now = time.time()
            return jsonify({
                "ok": True,
                "coins": rpc_result[0].get("coins", user["coins"] + final_coins),
                "season_coins": rpc_result[0].get("season_coins", user.get("season_coins", 0) + season_earned),
                "sprint_coins": rpc_result[0].get("sprint_coins", user.get("sprint_coins", 0) + sprint_earned),
                "energy": rpc_result[0].get("energy", new_energy),
                "earned": final_coins,
                "tap_power": tap_power,
        ...
        # ─── v3.1: Fallback to old method if RPC fails ────────────────────
        logger.warning(f"[TAP] {wallet[:8]}... RPC tap_earn_atomic failed, using fallback (race condition possible)")
    ...
    try:
        ...
        if rpc_result:
            ...
            return jsonify({
                "ok": True,
                "coins_earned": bonus,
                "coins": rpc_result[0].get("coins", user["coins"] + bonus),
                "season_coins": rpc_result[0].get("season_coins", user.get("season_coins", 0) + bonus),
                "streak": streak,
                "method": "atomic_rpc"
            })
...
# ─── Startup banner ──────────────────────────────────────────────────────────
print("=" * 60)
print("  Arena API v3.1 - Atomic Coin Operations")
