"""
Arena MiniApp API v3.2 - Energy Fix + Full Tap Logging
========================================================
Changes vs v3.1:
  #1 CRITICAL: Fixed "tap fantasma" — energy=0 now returns no_energy:true flag
  #2 CRITICAL: Added taps_accepted to ALL tap responses (frontend must use this)
  #3 CRITICAL: [TAP] log for EVERY request (even rejected/no-energy taps)
  #4 MEDIO:    Frontend must check: if no_energy → do NOT subtract locally
  All previous v3.1 fixes included (atomic RPC, server-side coin calc, etc.)
"""
...
@app.route("/")
def index():
    return "Arena API v3.2 - Energy Fix + Full Tap Logging", 200

@app.route("/health")
def health():
    return jsonify({"status": "ok", "version": "3.2", "atomic_rpc": True}), 200
...
def register_user():
    try:
        ...
        if ref and ref != wallet:
            ...
            if ru:
                # Use atomic RPC for referral bonus if available
...
def record_taps():
    try:
        ...
        logger.info(f"[TAP] {wallet[:8]}... request taps={taps_sent}, energy={current_energy}, accepted={actual_taps}")

        if actual_taps <= 0:
            server_now = time.time()
            logger.info(f"[TAP] {wallet[:8]}... REJECTED no energy (0/{taps_sent} sent)")
            return jsonify({
                "ok": True, "coins": user["coins"],
                "season_coins": user.get("season_coins", 0),
                "sprint_coins": user.get("sprint_coins", 0),
                "energy": 0, "earned": 0, "taps_accepted": 0,
                "tap_power": tap_power, "server_ts": server_now,
                "no_energy": True
            })

        # Server calculates coins from tap timestamps + combo
        ...
        # Calculate season and sprint additions
        season_earned = 0
        sprint_earned = 0
        if get_active_status("seasons"): season_earned = final_coins
        if get_active_status("sprints"): sprint_earned = final_coins

        # ─── v3.1: Try ATOMIC RPC first ──────────────────────────────────
        rpc_result = sb_rpc("tap_earn_atomic", {
            "p_wallet": wallet,
            "p_coins_earned": final_coins,
            "p_energy_used": actual_taps,
            "p_season_earned": season_earned,
            "p_sprint_earned": sprint_earned
        })

        if rpc_result is not None:
            # RPC succeeded - atomic update done in DB
            logger.info(f"[TAP] {wallet[:8]}... +{final_coins} coins, -{actual_taps} energy (RPC atomic) [{actual_taps}/{taps_sent}]")
            server_now = time.time()
            # READ FRESH data from DB after atomic update (not stale local values)
            fresh = get_user_by_wallet(wallet)
            if fresh:
                return jsonify({
                    "ok": True,
                    "coins": fresh.get("coins", 0),
                    "season_coins": fresh.get("season_coins", 0),
                    "sprint_coins": fresh.get("sprint_coins", 0),
                    "energy": fresh.get("energy", 0),
                    "earned": final_coins, "taps_accepted": actual_taps,
                    "tap_power": tap_power,
                    "server_ts": server_now,
                    "method": "atomic_rpc"
                })
            else:
                # Fallback if fresh read fails
                new_coins = user.get("coins", 0) + final_coins
                new_season = user.get("season_coins", 0) + season_earned
                new_sprint = user.get("sprint_coins", 0) + sprint_earned
                return jsonify({
                    "ok": True,
                    "coins": new_coins,
        ...
        # ─── v3.2: Fallback to old method if RPC fails ────────────────────
        logger.warning(f"[TAP] {wallet[:8]}... RPC FAILED, fallback [{actual_taps}/{taps_sent}] (race possible)")
        new_coins = user.get("coins", 0) + final_coins
        new_season = user.get("season_coins", 0) + season_earned
        new_sprint = user.get("sprint_coins", 0) + sprint_earned
        ...
        logger.info(f"[TAP] {wallet[:8]}... +{final_coins} coins, -{actual_taps} energy (FALLBACK) [{actual_taps}/{taps_sent}]")
        server_now = time.time()
        return jsonify({
            "ok": True, "coins": new_coins, "season_coins": new_season,
            "sprint_coins": new_sprint, "energy": new_energy, "earned": final_coins,
            "taps_accepted": actual_taps, "tap_power": tap_power, "server_ts": server_now,
            "method": "fallback"
        })
    except Exception as e:
...
def claim_daily():
    try:
        ...
        if ref:
            ...
            if ru:
                p = int(bonus * 0.05)
                # Try atomic RPC for referral passive
...
# ─── Startup banner ──────────────────────────────────────────────────────────
print("=" * 60)
print("  Arena API v3.2 - Energy Fix + Full Tap Logging")
print("  RPC functions: tap_earn_atomic, claim_daily_atomic,")
print("                  add_referral_passive")
print("  FIX: no_energy flag + taps_accepted in all responses")
print("=" * 60)
