"""
Arena MiniApp API v3.3 - Energy Regen Persistence Fix
=====================================================
Changes vs v3.2:
  #1 CRITICAL: Fixed energy regen loss — RPC now writes final energy (regen - taps)
               instead of subtracting taps from raw DB value (which was 0)
  #2 CRITICAL: tap_earn_atomic now takes p_new_energy (final value) not p_energy_used
  #3 MEDIO:    All v3.2 fixes included (no_energy flag, taps_accepted, full logging)

ROOT CAUSE of energy loss:
  DB stored energy=0 but calc_energy_regen showed 87.
  RPC did: GREATEST(0, 0 - 10) = 0 and reset timer.
  Fix:  RPC now directly sets energy = p_new_energy (server-calculated).
"""
...
@app.route("/")
def index():
    return "Arena API v3.3 - Energy Regen Fix", 200

@app.route("/health")
def health():
    return jsonify({"status": "ok", "version": "3.3", "atomic_rpc": True}), 200
...
def record_taps():
    try:
        ...
        new_energy = max(0, current_energy - actual_taps)
        ...
        # ─── v3.3: Try ATOMIC RPC — passes final energy (after regen) ──────
        rpc_result = sb_rpc("tap_earn_atomic", {
            "p_wallet": wallet,
            "p_coins_earned": final_coins,
            "p_new_energy": new_energy,
        ...
        if rpc_result is not None:
            ...
            logger.info(f"[TAP] {wallet[:8]}... +{final_coins} coins, energy {current_energy}->{new_energy} (RPC atomic) [{actual_taps}/{taps_sent}]")
            server_now = time.time()
...
# ─── Startup banner ──────────────────────────────────────────────────────────
print("=" * 60)
print("  Arena API v3.3 - Energy Regen Persistence Fix")
print("  RPC functions: tap_earn_atomic (p_new_energy),")
print("                  claim_daily_atomic, add_referral_passive")
print("  FIX: Energy regen now persisted correctly")
print("=" * 60)
